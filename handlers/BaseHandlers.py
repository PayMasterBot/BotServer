...
from keyboards import get_main_kb, get_expenses_kb


async def cmd_start(message: types.Message):
    await message.answer("Выберите раздел:", reply_markup=get_main_kb())
...
async def back_handler(message: types.Message, state: FSMContext):
    ...
    if message.text == "подписки":
        ...
        await message.answer("Меню трат", reply_markup=get_expenses_kb())


async def expenses_handler(message: types.Message):
    await message.answer("Раздел трат:", reply_markup=get_expenses_kb())
